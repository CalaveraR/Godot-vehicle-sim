...
func apply_to_suspension(
	...
	var lateral_force_ratio = abs(patch.total_force.x) / max(1.0, patch.total_force.y)
	suspension_system.lateral_deformation = lateral_force_ratio * tire_width * 0.1

	suspension_system.update_effective_radius(0.0, patch.max_pressure)
